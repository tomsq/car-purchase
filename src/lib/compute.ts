...
function phevEnergyCost(s: Scenario, totalKm: number): number {
...
export function computeScenario(s: Scenario, globals: Globals): CostBreakdown {
  const totalKm = globals.annualKm * globals.horizonYears
  ...
  const total =
    energy + leaseFee + maintenance + insurance + tires + roadTax + replacementCar
